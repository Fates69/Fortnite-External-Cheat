...
#define OFFSET_UWORLD 0x9412AA0 //48 8b 0d ? ? ? ? 48 85 c9 74 30 e8 ? ? ? ? 48 8b f8 0x9412A2
#define OFFSET_PLAYERCONTROLLER 0x30 //48 8b 0d ? ? ? ? 48 85 c9 74 30 e8 ? ? ? ? 48 8b f8 0x9412A2
#define OFFSET_TEAMINDEX 0xEC0 //48 8b 0d ? ? ? ? 48 85 c9 74 30 e8 ? ? ? ? 48 8b f8 0x9412A2
...
{
	DWORD_PTR bonearray;
	//	bonearray = read<DWORD_PTR>(DriverHandle, mesh + 0x4A8); //0x480
	bonearray = read<DWORD_PTR>(DriverHandle, processID, mesh + 0x4A8);
	if (bonearray == NULL)
	{
		//bonearray = RPM<DWORD_PTR>(fortnite_dirbase, mesh + 0x4A8 + 0x10);
		bonearray = read<DWORD_PTR>(DriverHandle, processID, mesh + 0x4A8 + 0x10);
	}
	//return RPM<FTransform>(fortnite_dirbase, bonearray + (index * 0x30));
...
Vector3 GetBoneWithRotation(DWORD_PTR mesh, int id)
{
	...
	//	FTransform ComponentToWorld = RPM<FTransform>(fortnite_dirbase, mesh + 0x1C0);
...
void cache()
{
	while (true)
	{
		...
		Uworld = read<DWORD_PTR>(DriverHandle, processID, base_address + 0x93538b0);
...
int main(int argc, const char* argv[])
{
	...
	while (hwnd == NULL)
	{                  
		SetConsoleTitle("Fates#8967 PASTED CHEAT");
		printf("Cheat will automatically inject once in Fortnite!");
		Sleep(2000);
		system("cls");
		XorS(wind, "Fortnite  ");
...
#ifdef DEBUG
	printf("\n\n  [+] Injected Succesfully! Do not close this window!");
#endif // DEBUG
...
void xInitD3d()
{
	...
	style->Colors[ImGuiCol_Text] = ImVec4(255, 0, 0, 255);
	style->Colors[ImGuiCol_TextDisabled] = ImVec4(0.50f, 0.50f, 0.50f, 1.00f);
	style->Colors[ImGuiCol_WindowBg] = ImColor(255, 254, 255);
	style->Colors[ImGuiCol_ChildBg] = ImVec4(1.00f, 1.00f, 1.00f, 0.00f);
	style->Colors[ImGuiCol_PopupBg] = ImVec4(0.08f, 0.08f, 0.08f, 0.94f);
	style->Colors[ImGuiCol_Border] = ImVec4(0.43f, 0.43f, 0.50f, 0.50f);
	style->Colors[ImGuiCol_BorderShadow] = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
	style->Colors[ImGuiCol_FrameBg] = ImColor(217, 217, 217);
	style->Colors[ImGuiCol_FrameBgHovered] = ImColor(217, 217, 217);
	style->Colors[ImGuiCol_FrameBgActive] = ImColor(217, 217, 217);
	style->Colors[ImGuiCol_TitleBg] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_TitleBgActive] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_MenuBarBg] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.02f, 0.02f, 0.02f, 0.53f);
	style->Colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.16f, 0.29f, 0.48f, 0.54f);
	style->Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(0.41f, 0.41f, 0.41f, 1.00f);
	style->Colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(0.51f, 0.51f, 0.51f, 1.00f);
	style->Colors[ImGuiCol_CheckMark] = ImVec4(255, 0, 0, 255);
	style->Colors[ImGuiCol_SliderGrab] = ImVec4(0, 0, 0, 255);
	style->Colors[ImGuiCol_SliderGrabActive] = ImVec4(0, 0, 0, 255);
...
void DrawESP() {
	DrawCircle(ScreenCenterX, ScreenCenterY, AimFOV, &Col.white, 25);
...
void render() {
	...
	if (ShowMenu)
	{                 
		XorS(menu, "Fates#8967 CHEAT [ MENU KEY : INSERT ]");
		...
		ImGui::SliderInt(aim6.decrypt(), &AimFOV, 90, 1000);
		ImGui::End();
	}
